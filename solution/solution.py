...
def call_openai(
    ...
    client = OpenAI(
        ...
        base_url=os.getenv("OPENAI_BASE_URL"),  # None => mặc định endpoint OpenAI
    )
...
def compare_models(prompt: str) -> dict:
    ...
    model_response, model_latency = call_openai(prompt)
    mini_response, mini_latency = call_openai_mini(prompt)

    output_tokens = len(model_response.split()) / 0.75
    gpt4o_cost_estimate = (output_tokens / 1000) * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]

    return {
        "gpt4o_response": model_response,
        "mini_response": mini_response,
        "gpt4o_latency": model_latency,
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # Model lạ (vd. model Mistral) hoặc offline không tải được encoding
        # -> ước lượng dự phòng: trung bình 1 token ≈ 4 ký tự
